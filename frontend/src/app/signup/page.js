...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 relative">
      ...
      >
        <h2 className="text-3xl font-bold text-center mb-2 text-white">Initialize ID</h2>
        <p className="text-center text-crypto-muted mb-8">Create a new secure trading profile.</p>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-crypto-muted mb-2">Operator Name</label>
            <div className="relative">
              <User className="absolute left-3 top-3 text-crypto-muted w-5 h-5" />
              <input
                type="text"
                required
        ...
        <p className="text-center mt-6 text-crypto-muted text-sm">
          Already have an ID?{" "}
          <Link href="/login" className="text-crypto-primary hover:underline">
            Access Terminal
          </Link>
        </p>
      </motion.div>
